...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      title: computed(() => {
        const websiteConfig = appStore.websiteConfig
        const currentLocale = locale.value
        
        // 根据语言选择显示中文名称或英文名称
        if (currentLocale === 'en' && websiteConfig.englishName) {
          return websiteConfig.englishName
        }
        
        return websiteConfig.websiteTitle || websiteConfig.name || metaStore.title
      }),
