    ...
    <div class="reply flex flex-col flex-wrap-reverse w-full max-w-full-calc" style="width: fit-content">
      ...
      <div class="justify-between" style="text-align: right">
        <button
          ...
          <span class="text-center flex-grow commit">{{ t('comment.reply_submit') }}</span>
        </button>
        <button
          @click="CancelReply"
          id="submit-button"
  ...
  setup(props, { emit }) {
    ...
    const saveReply = () => {
      if (userStore.userInfo === '') {
        proxy.$notify({
          title: 'Warning',
          message: t('comment.need_login_reply'),
          type: 'warning'
        })
        return
      }
      ...
      api.saveComment(params).then(({ data }) => {
        if (data.flag) {
          ...
          if (isCommentReview) {
            ...
            proxy.$notify({
              title: 'Success',
              message: t('comment.success_reply'),
              type: 'success'
            })
          }
