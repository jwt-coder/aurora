<template>
  <n-space vertical size="large">
    ...
    <n-form inline>
      <n-form-item label="关键词">
        <n-input v-model:value="keywords" placeholder="请输入模块名或描述" clearable style="width: 200px" @keyup.enter="handleSearch" />
      </n-form-item>
      <n-form-item>
  ...
  <!-- 查看详情对话框 -->
  <n-modal v-model:show="isCheckDialog" preset="card" title="详细信息" style="width: 600px">
    <n-descriptions v-if="currentLog" label-placement="left" :column="1">
      <n-descriptions-item label="操作模块">{{ currentLog.optModule }}</n-descriptions-item>
      <n-descriptions-item label="请求地址">{{ currentLog.optUri }}</n-descriptions-item>
      ...
      <n-descriptions-item label="请求参数">
        <n-code :code="currentLog.requestParam || '无'" language="json" />
      </n-descriptions-item>
      <n-descriptions-item label="返回数据">
        <n-code :code="currentLog.responseData || '无'" language="json" />
      </n-descriptions-item>
      <n-descriptions-item label="操作人员">{{ currentLog.nickname }}</n-descriptions-item>
    </n-descriptions>
  </n-modal>
...
import { NButton, NSpace, NTag, NPopconfirm, NCode, NDescriptions, NDescriptionsItem, NIcon, NPagination, useMessage, useDialog } from 'naive-ui'
import { TrashOutline } from '@vicons/ionicons5'
import dayjs from 'dayjs'
import { getOperationLogsApi, deleteOperationLogsApi } from '@/api/log'
...
function fetchLogs() {
  ...
  getOperationLogsApi(params).then(res => {
...
function handleDelete(id) {
  deleteOperationLogsApi([id]).then(() => {
...
function handleBatchDelete() {
  ...
  deleteOperationLogsApi(selectedIds.value).then(() => {
    message.success('删除成功')
